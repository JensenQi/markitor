(function (domGlobals) {
    ...
    let window = {};
    let global$2 = window;
    let _self = typeof window !== 'undefined' ? window : typeof WorkerGlobalScope !== 'undefined' && domGlobals.self instanceof WorkerGlobalScope ? domGlobals.self : {};
    let Prism = function () {
        ...
        let _ = _self.Prism = {
            ...
            highlightAll: function (async, callback) {
                let elements = domGlobals.document.querySelectorAll('code[class*="language-"], [class*="language-"] code, code[class*="lang-"], [class*="lang-"] code');
            ...
            highlightElement: function (element, async, callback) {
                ...
                if (async && _self.Worker) {
                    let worker = new domGlobals.Worker(_.filename);
                        ...
                        for (let j = 0; j < patterns.length; ++j) {
                            ...
                            for (let i = 0; i < strarr.length; i++) {
                                ...
                                if (match) {
                                    ...
                                    let from = match.index - 1 + lookbehindLength;
                                    match = match[0].slice(lookbehindLength);
                                    let len = match.length, to = from + len, before = str.slice(0, from + 1),
                                    ...
                                    let wrapped = new Token(token, inside ? _.tokenize(match, inside) : match, alias);
                                    args.push(wrapped);
                                    if (after) {
                                        args.push(after);
                                    }
        ...
        let Token = _.Token = function (type, content, alias) {
            this.type = type;
        ...
        if (!_self.document) {
            ...
            _self.addEventListener('message', function (evt) {
                let message = JSON.parse(evt.data), lang = message.language, code = message.code,
    ...
    let some = function (a) {
        ...
        let me = {
            ...
            equals_: function (o, elementEq) {
                return o.fold(false, function (b) {
    ...
    let getSelectedCodeSample = function (editor) {
    ...
    let CodeSample = {
        ...
        insertCodeSample: (editor, language, code) => {
            editor.undoManager.transact(function () {
                let node = getSelectedCodeSample(editor);
        ...
        getCurrentCode: editor => getSelectedCodeSample(editor).fold(() => '', n => n.textContent)
    };

    let Settings = {getLanguages: editor => editor.settings.codesample_languages};
    ...
    let Languages = {
        getLanguages: editor => {
            ...
            let customLanguages = Settings.getLanguages(editor);
            return customLanguages ? customLanguages : defaultLanguages;
        },
        getCurrentLanguage: (editor, fallback) => {
            return CodeSample.getSelectedCodeSample(editor).fold(() => fallback, n => {
                let matches = n.className.match(/language-(\w+)/);
    ...
    let openDialog = editor => {
        let languages = Languages.getLanguages(editor);
        let defaultLanguage = head(languages).fold(() => '', l => l.value);
        let currentLanguage = Languages.getCurrentLanguage(editor, defaultLanguage);
        let currentCode = CodeSample.getCurrentCode(editor);
    ...
    let setup = editor => {
        ...
        editor.on('SetContent', function () {
            let unprocessedCodeSamples = $('pre').filter(Utils.trimArg(Utils.isCodeSample)).filter(function (idx, elm) {
        ...
        editor.ui.registry.addToggleButton('highlight', {
            ...
            onSetup: api => {
                let nodeChangeHandler = () => {
                    ...
                    let isCodeSampleSelection = editor.dom.is(node, 'pre.language-markup');
                    api.setActive(isCodeSampleSelection);
                };
